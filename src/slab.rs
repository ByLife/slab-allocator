...
impl SlabAllocator {
    ...
    pub fn init(&mut self) {
        ...
        for (pool_idx, &size) in SIZES.iter().enumerate() {
            let chunks_in_pool = 4096 / (3 * size);
            self.free_counts[pool_idx] = chunks_in_pool;
            
            for _ in 0..chunks_in_pool {
    ...
    pub fn alloc(&mut self, layout: Layout) -> Result<*mut u8, AllocError> {
        ...
        let start_idx = pool_idx * (64 / SIZES.len());
        let end_idx = start_idx + (64 / SIZES.len());
